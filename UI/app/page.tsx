...
export default function Home() {
  const [params, setParams] = useState<VillesParams>({ page: 1, limit: 15, sort: "score", order: "DESC", scores: false, departement: "" });

  let { data, isLoading } = useSWR(
    `${process.env.NEXT_PUBLIC_API_HOST}/villes?page=${params.page}&limit=${params.limit}&sort=${params.sort}&order=${params.order}&scores=${params.scores}&departement=${params.departement}`,
    fetcher,
  ...
  const getColumnCell = (ville: any, key: string, score: string, unit: string = "") => (
    <TableCell key={key}>
      <Chip className="border-none" color={getScoreColor(ville[score])} radius="sm" size="sm">
        ...
        >
          {(ville: Ville) => (
            <TableRow key={ville.code_insee}>
              ...
              <TableCell>
                <Chip className="border-none" color={getScoreColor(ville.score_delinquance)} radius="sm" size="sm">
              ...
              <TableCell>
                <Chip className="border-none" color={getScoreColor(ville.score_geo_grande_ville_distance)} radius="sm" size="sm">
